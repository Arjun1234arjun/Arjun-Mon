...
Asena.addCommand({ pattern: 'insta ?(.*)', fromMe: false, desc: IG_DESC }, async (message, match) => {
    ...
    if (type === 'image') { await message.sendMessage(data, MessageType.image, { caption: "```ẉa.me/➳ᴀͥᴍͭᴀᷤʟꜱͫᴇͤʀ```" }) }
    else if (type === 'video') { await message.sendMessage(data, MessageType.video, { caption: "```ẉa.me/➳ᴀͥᴍͭᴀᷤʟꜱͫᴇͤʀ```" }) }
});
//const axios = require('axios')
