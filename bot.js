...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('credentials-updated', async () => {
        console.log(
            chalk.green.bold('✅ Amalser kayarittund 😎')
        );
        console.log(
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                await conn.sendMessage(conn.user.jid, '*-- HATA RAPORU [WHATSASENA] --*' + 
                                    ...
                                    '\n_Hatayı https://chat.whatsapp.com/DHYuPadEvEw4wvs93NtL6n._\n\n' +
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [Amalser] --*' + 
                                    '\n*Amalser an error has occurred!*'+
                                    '\n*This error log may include your number or the number of an opponent. Please be careful with it!*' +
                                    '\n*1st join our Group other wise Erorr will get.*' +
                                    '\n*Aslo you can join our support group:*  https://chat.whatsapp.com/DHYuPadEvEw4wvs93NtL6n' +
                                    '\n*This message should have gone to your number (saved messages).*\n\n' +
                                    '*Error:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false}
                                );
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [Amalser] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*🙇🏻 Sorry, I Couldnt Read This Error! 🙇🏻(erorr👀Dont worry anikum ariyilla aevideya erorr n*' +
